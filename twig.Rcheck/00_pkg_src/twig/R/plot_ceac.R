...
plot_ceac <- function(rewards_sim, wtp_range, col_names = c("cost", "utility")){
  # Check if rewards_sim is an array
  if(!is.array(rewards_sim)){
    stop("rewards_sim should be an array. If you have used verbose = TRUE in run_twig, make sure to rerun run_twig with verbose = FALSE before calling plot_ceac.")
  }
  # if number of dimensions is not 3, return an error
  if(length(dim(rewards_sim)) != 3){
    stop("rewards_sim should be a 3D array with dimensions decision, reward (cost and utility), and simulation (sim).")
  }

...
The Markov model involves a cost-effectiveness example with 4 decisions and 4 states. For more details on the model, please refer to the reference above. First we define the twig object and the layers. 
```{r}
twig_obj <- twig() + 
  decisions("StandardOfCare", "StrategyA", "StrategyB", "StrategyAB") + 
  states(names = c("H", "S1", "S2", "D"), init_probs = c(1,0,0,0)) +  
  event(name = "get_event",  
                options = c("recover", "getsick", "progress", "die", "stay"), 
                probs = c(pRecover(state), pGetSick(state), pProgress(state, decision), pDie(state), Inf), 
                transitions = c("H", "S1", "S2", "D", "stay")) +  
...
Then, we build the twig Markov model by using the `twig_gen_model_function()` function.
...
```{r}
n_cycles <- 75
model_res <- my_markov_model(twig_obj, params, return_trace = T)
```

...
twig_obj <- twig() + 
  ...
  states(names = c("H", "S", "D"), initial_probabilities = c(1,0,0)) + 
  event(name = "only_event",  
        options = c("get_sick", "die", "stay"), 
        probabilities = c(pGetSick(state), pDie(decision, state), Inf), 
        transitions = c("S", "D", "current_state")) +  
...
-   `states(...)`: defines the Markov twig. There must be only one of these per model. The states funciton takes in 3 arguments:
    ...
    -   initial_probabilities for initial probabilities, and
...
-   `event(...)`: is the most powerful feature in twig and it governs how the states are linked through events. In the Grammar of Modeling we distinguish between a *state* and an *event*. We use a noun for a `state` to indicate where the cohort stays a full cycle (e.g., one year), and use a verb to refer to an `event`, which in a Markov world is instantaneous and doesn't take any time at all (e.g., `get_sick` for getting sick). There has to be at least one `event` twig, but there can be more events that can be chained together as we explain below. The event mapping takes in four arguments: name, options, outcomes, and probabilities. We will explain each of these separately:
    ...
    -   `probabilities = c(pGetSick(state), pDie(decision, state), Inf))`: these are the probabilities of the correspondign options occuring. A powerful feature of twig is that it allows these probabilities to be conditional on other model components. Here we define the probability of only_event==get_sick by calling pGetSick(state) which is dependent on the state. Similarly, the probability of only_event==die is defined by calling `pDie(decision, state)` which we tell twig that is dependent on the state and the decision. `pGetSick` and `pDie` are user-defined functions that we describe below. The `Inf` is used to tell twig to compute the complementary probability for the probability of staying in the same state: (i.e., 1-pGetSick(state)-pDie(state)) to reduce the amount of typing needed.
    -   `transitions = c("S", "D", "current_state")`: defines the transitions of each one of the event values. getting sick ends up in the S state, dying ends up in the D state, and staying will return to whichever state the cohort was in using the special keyword `"current_state"`.
-   `payoffs(cost = cost(decision, state))`: defines a single payoff hers. Here we only have a cost payoff that is dependent on the state and decision. More than one payoff can be described here, but only a single payoff twig is allowed. And, similar to the probabilities argument these can be a function of other model components, such as decision, state, cycles, events, tunnels, ...etc
...
```{r}
states(names = c("H","S","D"), initial_probabilities = c(0.7,0.2,0.1))
```
...
pH0 <- 0.7
pS0 <- 0.2
states(names = c("H","S","D"), initial_probabilities = c(pH0, pS0, Inf))
```

To run the model with this updated probabilities, we need to add it as a new `init_prob` layer to the `twig` code.
...
twig_obj <- twig() + 
  ...
  event(name = "only_event",  
              options = c("get_sick", "die", "stay"), 
              probabilities = c(pGetSick(state), pDie(decision, state, cycle), Inf), 
              transitions = c("S", "D", "current_state")) +  
...
twig_obj <- twig() + 
  decisions("StandardOfCare", "StrategyA") +
  states(names = c("H", "S", "D"), initial_probabilities = c(1,0,0), max_cycle_in_states = c(1,3,1)) +  #
  event(name = "only_event",  
              options = c("get_sick", "die", "stay"), 
              probabilities = c(pGetSick(state), pDie(decision, state, cycle_in_state), Inf), 
              transitions = c("S", "D", "current_state")) + #
...
twig_obj <- twig() + 
  ...
  states(names = c("Alive", "Dead"), initial_probabilities = c(1,0)) + 
  event(name = "get_sick",  
              options = c(TRUE, FALSE), 
              probabilities = c(0.05, Inf), 
...
twig_obj <- twig() + 
  ...
  event(name = "get_sick",  
    ...
    event(name = "die",  
              options = c(TRUE, FALSE), 
              probabilities = c(pDie(state, get_sick), Inf), 
              transitions = c("Dead", "current_state")) +  
...
In this Vignette we went over the basic features of the twig library for building and running Markov models. We illustrated these features one at a time, and show how to define decisions, states, events, payoffs. In addition, we show how to change the number of cycles, the discounting rates. Furthermore, we show how to make transition probabilities and payoffs a function of decisions, states cycles, number of cycles in a state (tunnel) and previous events.

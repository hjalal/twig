...
-   `states(...)`: defines the Markov twig. There must be only one of these per model. The states funciton takes in 3 arguments:
    ...
    -   optional tunnel_lengths which defines the maximum number of tunnel states per state. This option is omitted here, so the function uses the default of 1.
...
mytwig <- twig() + 
  decisions("StandardOfCare", "StrategyA") +
  states(names = c("H", "S", "D"), init_probs = c(1,0,0), tunnel_lengths = c(1,3,1)) +  #
  event(name = "only_event",  
              scenarios = c("get_sick", "die", "stay"), 

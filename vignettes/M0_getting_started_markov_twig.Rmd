...
This will create the `twig_env` which contains the model equations and formulae and can be examined, and as stated in the generated note, this function also produces a function representation of the Markov model. The default name of this function is `my_markov_model.`

Let's examine the content of these two object. First, let's check the model structure

```{r}
twig_env # returns the model structure including the formulae before evaluation
```
...
```{r}
model_results <- my_markov_model(twig_env, return_trace = T)
model_results$Trace
```
...
my_markov_model(twig_env, return_cycle_payoffs=TRUE)
```
...
```{r}
twig_env <- twig_gen_model_function(twig_env)
model_results <- my_markov_model(twig_env, return_transition_prob = T, return_state_payoffs = T)
```
...
twig_env <- twig_gen_model_function(twig_env)
my_markov_model(twig_env,return_transition_prob=TRUE,return_state_payoffs=TRUE,return_trace=TRUE,return_cycle_payoffs=TRUE,return_payoff_summary=TRUE)
...
twig_env <- twig_gen_model_function(twig_env)
my_markov_model(twig_env,params=params)

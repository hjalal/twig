...
The outcomes may be slightly different from the tutorial. The reason is that the tutorial applies a within cycle correction and also assumes that transition rewards happen after the transition.  twig links the transition rewards to the events so they occur during each cycle. 
...
twig_obj <- twig() + # for illustration it is 75 in the tutorial 
  ...
  event(name = "get_event",  
        options = c("recover", "getsick", "progress", "stay"), 
...
The `twig()` function defines the model type as a Markov model and number of cycles as 75. The `decision()` function defines the treatment strategies.  The `initial_probs()` defines the initial probs. Here we only define the "Moderate" states as the starting state with probability equals 1. The other states will be set to 0. 
...
The `cycle_in_state` is a special keyward that consists of the prefix `cycle_in_` followed by one of the state names.  This keyword tells twig which state to expand by adding additional tunnel states. You can create other tunnels by adding `cycle_in_[State]`.  Note, adding tunnel states expands the state space and can be computationally expensive. 

The `"stay"` is a build-in function that instructs twig to compute the probability of a state returning to itself.
...
The model has six user-defined functions: four for computing probs `pRecover()`, `pGetSick()`, `pProgress()` and `pDie()`, and 2 for payoffs `cost()` and `utility()`.  
...
Then, we build the twig Markov model by using the `twig_build()` function.

The output is the structure of the model including the formulae.  

To evaluate the model, we run `twig_evaluate()` which returns the numerical outcomes. 
```{r}
...
model_results <- my_markov_model(twig_obj, params, return_trace = T)
...
The warning just indicates that no parameters were passed, so twig uses the values in the global environment.  For analyses, such as probabilistic sensitivity analysis (PSA), we can pass the parameters as a list `param`. 

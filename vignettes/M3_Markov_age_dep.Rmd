...
```{r}
n_age_init <- 25 - 1 # age at baseline - twig starts from cycle 1 instead of 0 as in the tutorial. 
...
The output is the structure of the model including the formulae using `twig_gen_model_function()` and then calling the generated function `my_markov_model()`.  
To evaluate the model, we run `twig_evaluate()` which returns the numerical outcomes. 
```{r}
n_cycles <- 10
twig_obj <- twig_gen_model_function(twig_obj)
model_res <- my_markov_model(twig_obj, return_trace = T)
#model_res
```
